...
- **Sicurezza — `scripts/security_rls.sql` ESEGUITO**: attivata RLS su `reviews` (era disattivata!); `points` non più scrivibile dal client (revoke UPDATE tabella `profiles`, grant solo `push_token`; `update_user_points` reso SECURITY DEFINER)
...
- ⚠️ **Ancora da fare (fuori dal codice)**: restringere la chiave Google Maps su Cloud Console; verificare bucket `review-photos` + "Leaked password protection" in Auth; togliere il backup `service_areas_backup_dedup` a verifica fatta
