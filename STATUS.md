...
- **Pentest automatico** (`scripts/security_pentest.mjs`, ESEGUITO): 10/11 verdi. Ha trovato 2 vulnerabilità reali: (1) `push_token` esposto → CHIUSA con security_rls sez.3; (2) **upload anonimo sul bucket `review-photos`** → fix in `scripts/storage_security.sql` **DA ESEGUIRE** (diagnostica policy + upload solo authenticated; eliminare eventuale policy permissiva residua)
...
- ⚠️ **Ancora da fare (fuori dal codice)**: rieseguire `security_rls.sql` (sez.3 push_token) + eseguire `storage_security.sql`; attivare il secret di `notify-like`; restringere la chiave Google Maps su Cloud Console; "Leaked password protection" in Auth; togliere il backup `service_areas_backup_dedup` a verifica fatta
